...
export default async function SSRFetcher<T = unknown>(
  ...
  try {
    ...
    const setCookies = cookieHeader
      ? cookieHeader.split(/,(?=\s*[a-zA-Z0-9_\-]+\=)/)
      : null; // Robust splitting
    ...
    if (setCookies) {
      ...
      const getCookieOptions = (attrs: Record<string, any>) => ({
        ...
        maxAge: attrs["max-age"] ? parseInt(attrs["max-age"]) : 60 * 60 * 24, // Fallback: 1 day
...
async function handleTokenRefresh(
  ...
  try {
    ...
    const setCookies = refreshCookieHeader
      ? refreshCookieHeader.split(/,(?=\s*[a-zA-Z0-9_\-]+\=)/)
      : null; // Robust splitting
