...
export default function LanguageSwitcher({ isAsideOpen }: Props) {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger
        className={`${isAsideOpen ? "px-[10px]" : "justify-center"
      ...
      >
        ...
        />
        {isAsideOpen && (
          <span className="text-sm font-medium">{currentLocaleInfo?.label}</span>
        )}
        {isAsideOpen && (
          <IconBase
      ...
      <DropdownMenuContent
        className={`${isAsideOpen ? "w-[200px]" : "w-5"
      ...
      >
        {locales.map((locale) => (
          <DropdownMenuItem
            ...
            className={`${!isAsideOpen && activeLocale === locale.key
              ? "!bg-primary/40"
              : ""
